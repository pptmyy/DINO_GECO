...
def boxes_with_scores(density_map,tlrb,sort=False,validate=False):
    ...
    """
    从密度图和偏移量提取边界框
    dentisty_map 预测的物体中心热力图[B,1,H,W]
    tlrb: 预测的到上下左右边缘的距离(top,left,right,bottom) [B,4,H,W]
    """
    B,C,_,_ = density_map.shape

    score_map = density_map.sigmoid()
    pooled = F.max_pool2d(score_map,3,1,1)

    if validate:
        batch_thresh = torch.max(score_map.reshape(B,-1),dim=-1).values.view(B,C,1,1) / 8
    else:
        batch_thresh = score_map.flatten(1).median(dim=1).values.view(B,C,1,1)

    mask = (pooled == score_map) & (score_map > batch_thresh)
    ...
    for i in range(B):
        ...
        bbox_scores = score_map[i, mask[i]]
        bbox_logits = density_map[i, mask[i]]
        ...
        out_batch.append(
            {
                ...
                "boxes": bbox_xyxy.unsqueeze(0),
                "box_v": bbox_logits.unsqueeze(0)
            }
        )

        ref_points_batch.append(ref_points)

    max_points = max((points.shape[0] for points in ref_points_batch), default=0)
    ref_points = density_map.new_zeros((B, max_points, 2), dtype=torch.long)
    for i, points in enumerate(ref_points_batch):
        if points.numel() > 0:
            ref_points[i, : points.shape[0]] = points

    return out_batch,ref_points
...
def remove_small_box(boxlist,scores,threshold,max_proposal=-1):
    if threshold <=0:
        return boxlist
...
def compute_location(features):
    ...
    location_per_level = compute_location
    (
        height,width,1,features.device
    )
    location.append(location_per_level)
